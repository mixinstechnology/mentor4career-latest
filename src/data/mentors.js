...
//  console.log(data)
 ...
 const arr =data?.map((item,id)=>{ 
  if(item?.isVerified){
  return ({  id: item?.authUserId ?? item?.id, init: String(item?.firstName)?.slice(0,2), name: item?.firstName + ' ' + item?.lastName, type: item?.type, typeLabel: 'Final Year', role: item?.jobRole, org: item?.organizationName, stream: 'cse', focus: ['admissions', 'campus', 'placement'], rating: item?.rating ?? 4.9, reviews: item?.reviews ?? 212, sessions: item?.totalSessions ?? 340, price: item?.chargePerSession ?? 0, resp: '~2 hrs', bio: item?.bio})
 }
